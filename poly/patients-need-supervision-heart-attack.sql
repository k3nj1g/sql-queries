...
(
  SELECT  "patient_context". *AS  "patient_context",
         'heart-attack' AS  "category"
  FROM  "patient_context"
    INNER JOIN  "patientflag"
            ON ( "patient_context". "patient_id" = ( "patientflag". "resource" #>> '{subject,id}'))
           AND ( "patientflag". "resource" @@ 'flag.#.code.coding.#.code="A04.3"'::jsquery)
  WHERE jsonb_path_exists( "patient_context". "encounter_resource",'$.contained.code.coding ? (@.code starts with "I21" || @.code starts with "I22")')
)
