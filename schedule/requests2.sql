...
WITH grouped AS (
  SELECT prr.resource #>> '{derived,moshort}' org_name
    ...
    , concat(pr.resource#>>'{name,0,family}', ' ', regexp_replace((pr.resource#>>'{name,0,given,0}'), '([а-я])+', '.', 'g'), ' ', regexp_replace((pr.resource#>>'{name,0,given,1}'), '([а-я])+', '.', 'g')) doctor 
